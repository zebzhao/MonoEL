...
public class VSSlider: UIControl {
    ...
    public var thumbCenter: CGPoint {
        get {
            let thumbRect = slider.thumbRect(forBounds: bounds, trackRect: slider.trackRect(forBounds: bounds), value: slider.value)
            return CGPoint(x: thumbRect.midX + frame.minX, y: thumbRect.midY + frame.minY)
        }
    }
...
class InternalSlider: UISlider {
    ...
    var roundedValue: Float {
        get {
            return snapToValue(super.value, to: increment)
        }
    }
    ...
    @objc private func endSliding() {
        UIView.animate(withDuration: 0.2, animations: {
            self.setValue(self.roundedValue, animated: true)
        })
        sendActions(for: .valueChanged)
    }
}

fileprivate func snapToValue(_ value: Float, to increment: Float) -> Float {
    if increment == 0 {
        return value
    }
